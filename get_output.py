...
def get_json(x, product_data):
	...
	with open(f"json_data/amazon-data-{''.join([x + '-'for x in x.split()[:-1]] + x.split()[-1:])}.json", "w") as f:
...
def get_csv(x, product_data):
	...
	df.to_csv(f"csv_data/amazon-data-{''.join([x + '-'for x in x.split()[:-1]] + x.split()[-1:])}.csv", index=False)
